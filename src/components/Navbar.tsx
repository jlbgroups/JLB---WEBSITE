...
export default function Navbar() {
  ...
  const navLinkClass = ({ isActive }: { isActive: boolean }) =>
    `relative text-[13px] font-medium tracking-wide transition-colors duration-200 ${isActive
  ...
  return (
    <header
      className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled
          ? 'bg-white/80 backdrop-blur-xl shadow-[0_4px_30px_rgba(0,0,0,0.03)] border-b border-border/50'
    ...
    >
      <div className="max-w-container mx-auto px-6 flex items-center justify-between h-[68px]">
        ...
        <nav className="hidden lg:flex items-center gap-7">
          ...
          >
            <NavLink
              to="/services"
              className={({ isActive }) =>
                `relative flex items-center gap-1 text-[13px] font-medium tracking-wide transition-colors duration-200 ${isActive
                  ? 'text-forest after:absolute after:bottom-[-4px] after:left-0 after:right-0 after:h-[2px] after:bg-accent after:rounded-full'
                  : 'text-textPrimary hover:text-accent'
                }`
              }
            ...
            <div
              className={`absolute top-[calc(100%+8px)] left-1/2 -translate-x-1/2 bg-white border border-border rounded-xl shadow-xl py-1.5 w-64 z-50 transition-all duration-150 ${servicesOpen
        ...
        <div className="flex items-center gap-3">
          <Link
            to="/contact"
            className="hidden lg:inline-flex items-center gap-2 px-6 py-2.5 text-[13px] font-bold bg-forest text-white rounded-full hover:bg-forest-light hover:shadow-[0_4px_14px_rgba(11,19,43,0.25)] transition-all duration-300 group/btn"
          >
            Let's Connect
      ...
      {/* Mobile menu */}
      <div
        className={`lg:hidden fixed inset-x-0 top-[68px] bg-white border-t border-border transition-all duration-300 ${mobileOpen
        ...
        <div className="px-6 py-5 space-y-1">
          {[
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center justify-between py-3.5 text-[15px] font-medium border-b border-border-light last:border-0 transition-colors ${isActive ? 'text-accent' : 'text-textPrimary'
                }`
              }
