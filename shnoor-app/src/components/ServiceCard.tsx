...
export default function ServiceCard({ icon, title, description, link = '/services' }: ServiceCardProps) {
  return (
    <div className="group bg-white border border-border rounded-xl p-6 hover:border-accent/30 card-hover relative overflow-hidden">
      ...
      <div className="relative z-10">
        ...
        <p className="text-textSecondary text-[14px] leading-relaxed mb-5">
          {description}
        </p>
        <Link
          to={link}
          className="inline-flex items-center gap-1.5 text-[13px] font-semibold text-forest hover:text-accent hover:gap-2.5 transition-all duration-200"
        >
          <ArrowRight size={14} />
        </Link>
      </div>
